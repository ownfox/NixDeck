...
2. **EmuDeck (Ретро-эмуляция):**
   ...
   - Щелкните правой кнопкой мыши по файлу -> Свойства -> Права -> Установите галочку "Является выполняемым" (Is executable).
   - Запустите файл двойным кликом.

*Примечание: Такие программы как Flatseal, Webapp-manager или PortProton лучше всего устанавливать через магазин приложений (Discover) в десктопном режиме как Flatpak, так как они изначально не адаптированы под NixOS.*
